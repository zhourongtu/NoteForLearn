...
- 委托是什么
  - 委托是函数（方法）的容器
  - 用于 存储、传递 函数（方法）
  - 本质上是一个类，用来定义函数（方法）的类型（返回值、参数类型）
  - 不同的函数（方法）必须对应和各自“格式”一致的委托。
  - **语法**
    - ``` C#
      访问修饰符 delegate 返回值 委托名(参数列表)
      ```
    - 写在namespace（主要）和class语句块中
    - 不可重名（同一语句块内）
    - 默认public
  - 常用于1.作为类的成员、2.作为函数的参数。（特别像是Python的一个变量，用于函数式编程）
  - 可存储多个函数（委托 += func)，需要先初始化（空调用会报错）。（通过-=可以删， =null可以全移除）
- 系统定义好的委托： Action、Func<string, ...>参数+返回值、Action<int....>无返回值，带参数
  - 可以自己写泛型委托（类似于系统提供的）
